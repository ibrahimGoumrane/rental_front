...
import { Star, Shield, Check, Calendar, MessageSquare, User, X, MapPin, Flag, ChevronDown, ChevronUp, Minus, Plus, FileText, Play, Building, Home, Castle, BoxIcon, Palmtree } from 'lucide-react';
...
const HOUSE_RULE_LABELS: Record<string, string> = {
  ...
  no_overnight_guests: 'Overnight guests not listed in the reservation are not allowed',
...
const HOUSE_RULE_GROUPS = [{
  ...
  rules: ['respect_neighbors', 'no_parties', 'quiet_hours', 'reasonable_condition']
}, {
  id: 'guests',
  title: 'Guests & Occupancy',
  rules: ['registered_only', 'visitors_declared', 'max_occupancy', 'no_overnight_guests']
}, {
  id: 'local',
  ...
  id: 'smoking_pets',
  title: 'Smoking & Pets',
  rules: ['no_smoking', 'designated_smoking', 'alcohol_respectful', 'no_illegal_substances', 'no_pets', 'pets_approved', 'pets_quiet']
}, {
  id: 'property_safety',
  title: 'Property & Safety',
  rules: ['no_commercial', 'no_filming', 'furniture_moved', 'damage_responsibility', 'report_issues', 'reasonable_usage']
}];
export function PropertyDetails({
  ...
  const pricePerNight = parseInt(property.price.replace('$', '').replace(',', ''));
  const getDays = () => {
  ...
  const isAvailable = isDatesSelected && days > 0;
  const handleReserve = () => {
    if (!isDatesSelected) return;
    setIsReserved(true);
    setShowSuccessModal(true);
  };
  const updateGuests = (type: 'adults' | 'children' | 'infants', operation: 'inc' | 'dec') => {
    setGuests(prev => ({
      ...prev,
  ...
  const getPropertyTypeDisplay = () => {
    ...
    if (property.propertyType.primary) {
      parts.push(PROPERTY_TYPE_LABELS[property.propertyType.primary] || property.propertyType.primary);
    }
    if (property.propertyType.secondary) {
      parts.push(PROPERTY_TYPE_LABELS[property.propertyType.secondary] || property.propertyType.secondary);
    }
    if (property.propertyType.rentalMode) {
      parts.push(`(${RENTAL_MODE_LABELS[property.propertyType.rentalMode] || property.propertyType.rentalMode})`);
    }
    return parts.join(' · ') || property.stats.type;
  };
  // Get active house rules
  const getActiveRules = () => {
    if (!property.houseRules) return property.rules;
    return Object.entries(property.houseRules).filter(([_, isActive]) => isActive).map(([ruleId]) => HOUSE_RULE_LABELS[ruleId] || ruleId);
  };
  return <div className="max-w-7xl mx-auto px-6 md:px-12 py-12 bg-white/95 backdrop-blur-md rounded-t-3xl shadow-2xl mt-0 relative z-10">
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <div className=" pb-8">
            <div className="flex items-center justify-between mb-4">
              ...
              {isDatesSelected && <div className="flex items-center space-x-2">
          ...
          <div className="flex items-center justify-between py-6 border-b border-charcoal/10">
            ...
            <img src={property.host.avatar} alt={property.host.name} className="w-14 h-14 rounded-full object-cover border-2 border-sand" />
          </div>

          {/* Highlights */}
          <div className="py-6 border-b border-charcoal/10 space-y-6">
            {property.highlights.map((highlight, idx) => <div key={idx} className="flex items-start space-x-4">
          ...
          <div className="py-6 border-b border-charcoal/10">
            <div className={`prose prose-lg prose-charcoal max-w-none relative ${!isDescriptionExpanded ? 'max-h-[200px] overflow-hidden' : ''}`}>
              <p className="text-charcoal/80 leading-relaxed whitespace-pre-line">
                {property.description}
              </p>
              {!isDescriptionExpanded && <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-white to-transparent" />}
            </div>
            <button onClick={() => setIsDescriptionExpanded(!isDescriptionExpanded)} className="mt-4 flex items-center text-charcoal font-medium underline decoration-charcoal/30 hover:decoration-charcoal transition-all">
          ...
          <div className="py-8 border-b border-charcoal/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {property.amenities.slice(0, 6).map((item, idx) => <div key={idx} className="flex items-center space-x-4 p-3 rounded-lg hover:bg-charcoal/5 transition-colors">
          ...
          {property.houseRules && Object.keys(property.houseRules).length > 0 && <div className="py-8 border-b border-charcoal/10">
                <h3 className="font-serif text-2xl text-charcoal mb-6">
                  House Rules
                </h3>
                <div className="space-y-4">
                  {HOUSE_RULE_GROUPS.map(group => {
              const activeRulesInGroup = group.rules.filter(ruleId => property.houseRules?.[ruleId]);
              if (activeRulesInGroup.length === 0) return null;
              return <div key={group.id} className="border border-charcoal/10 rounded-xl overflow-hidden">
                    ...
                    duration: 0.3
                              ...
                              <div className="p-4 space-y-3 bg-white">
                                {activeRulesInGroup.map(ruleId => <div key={ruleId} className="flex items-start space-x-3 p-2">
          ...
          {(!property.houseRules || Object.keys(property.houseRules).length === 0) && property.rules && property.rules.length > 0 && <div className="py-8 border-b border-charcoal/10">
                <h3 className="font-serif text-2xl text-charcoal mb-6">
                  House Rules
                </h3>
                <ul className="space-y-3">
                  {property.rules.map((rule, idx) => <li key={idx} className="flex items-start space-x-3">
                      <Check className="w-5 h-5 text-warm-green flex-shrink-0 mt-0.5" />
          ...
          <div className="py-8 border-b border-charcoal/10">
            <div className="flex items-center justify-between mb-8">
              ...
              {!showRatingForm && <button onClick={() => setShowRatingForm(true)} className="px-4 py-2 border border-charcoal/20 rounded-lg text-sm font-medium hover:bg-charcoal hover:text-white transition-all">
                  Write a review
                </button>}
            </div>

            <AnimatePresence>
              {showRatingForm && <motion.div initial={{
              height: 0,
              opacity: 0
            }} animate={{
              height: 'auto',
              ...
              opacity: 0
                  ...
                  <RatingForm propertyId="1" onCancel={() => setShowRatingForm(false)} />
                </motion.div>}
            </AnimatePresence>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {property.reviews.map(review => <div key={review.id} className="space-y-4">
            ...
            <div className="bg-sand/30 p-8 rounded-xl border border-charcoal/5 shadow-sm">
              <div className="flex flex-col md:flex-row gap-8">
                <div className="flex flex-col items-center text-center space-y-2 min-w-[200px]">
                  <img src={property.host.avatar} alt={property.host.name} className="w-24 h-24 rounded-full object-cover shadow-md" />
                  <h4 className="font-serif text-xl text-charcoal mt-2">
                    {property.host.name}
                  </h4>
                  {property.host.verified && <div className="flex items-center text-warm-green text-sm font-medium">
                ...
                <div className="flex-1 space-y-4">
                  ...
                  {isReserved && <button onClick={() => navigate('/messages')} className="px-6 py-3 bg-charcoal text-white rounded-lg hover:bg-charcoal/90 transition-colors text-sm font-medium flex items-center">
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-8">
            <div className="bg-white p-6 rounded-xl shadow-xl shadow-charcoal/5 border border-charcoal/5">
              ...
              <div className="border border-charcoal/20 rounded-lg mb-4">
                <div className="grid grid-cols-2 border-b border-charcoal/20">
                  <div className="p-3 border-r border-charcoal/20">
                    ...
                    <input type="date" value={checkIn} onChange={e => setCheckIn(e.target.value)} className="w-full text-sm outline-none text-charcoal bg-transparent p-0" />
                  </div>
                  <div className="p-3">
                    <label className="block text-[10px] uppercase font-bold text-charcoal/60 mb-1">
                      Check-out
                    </label>
                    <input type="date" value={checkOut} onChange={e => setCheckOut(e.target.value)} className="w-full text-sm outline-none text-charcoal bg-transparent p-0" />
                  </div>
                </div>

                {/* Guest Dropdown */}
                <div className="relative">
                  <div className="p-3 cursor-pointer hover:bg-charcoal/5 transition-colors" onClick={() => setShowGuestsDropdown(!showGuestsDropdown)}>
                    <label className="block text-[10px] uppercase font-bold text-charcoal/60 mb-1">
                      Guests
                    </label>
                    <div className="flex justify-between items-center">
                      ...
                      <ChevronDown className={`w-4 h-4 text-charcoal transition-transform ${showGuestsDropdown ? 'rotate-180' : ''}`} />
                    </div>
                  </div>

                  {showGuestsDropdown && <div className="absolute top-full left-0 right-0 bg-white border border-charcoal/10 shadow-xl rounded-lg mt-1 p-4 z-20">
                      {['adults', 'children', 'infants'].map(type => <div key={type} className="flex items-center justify-between py-3 border-b border-charcoal/5 last:border-0">
                          <div>
                          ...
                          <div className="flex items-center space-x-3">
                            <button onClick={() => updateGuests(type as any, 'dec')} disabled={guests[type as keyof typeof guests] === 0 || type === 'adults' && guests.adults === 1} className="p-1 rounded-full border border-charcoal/20 hover:border-charcoal text-charcoal disabled:opacity-30 disabled:cursor-not-allowed">
                              <Minus className="w-4 h-4" />
                            </button>
                            <span className="w-4 text-center text-sm">
                              {guests[type as keyof typeof guests]}
                            </span>
                            <button onClick={() => updateGuests(type as any, 'inc')} className="p-1 rounded-full border border-charcoal/20 hover:border-charcoal text-charcoal">
              ...
              <button onClick={handleReserve} disabled={!isDatesSelected} className={`w-full font-bold py-3.5 rounded-lg transition-all shadow-lg mb-4 ${isDatesSelected ? 'bg-warm-green hover:bg-warm-green/90 text-white shadow-warm-green/20' : 'bg-charcoal/10 text-charcoal/40 cursor-not-allowed'}`}>
                {isDatesSelected ? 'Reserve' : 'Check availability'}
              </button>

              {!isDatesSelected && <p className="text-center text-sm text-charcoal/60 mb-2">
                  Select dates to check availability
                </p>}

              {isDatesSelected && <div className="space-y-3 pt-4 border-t border-charcoal/10">
      ...
      {/* Success Modal */}
      <AnimatePresence>
        {showSuccessModal && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          opacity: 0
        }} className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowSuccessModal(false)} />
            <motion.div initial={{
          opacity: 0,
          scale: 0.95,
          y: 20
