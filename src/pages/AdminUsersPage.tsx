...
import { Search, Download, Mail, Filter, ChevronDown, MoreVertical, CheckCircle, Clock, XCircle, Shield, Eye, MessageSquare, Ban, Key, FileText, Calendar, Home, MapPin, Activity, AlertCircle, X, RefreshCw, ChevronLeft, ChevronRight, UserCheck, Image } from 'lucide-react';
...
export function AdminUsersPage() {
  ...
  const [sortBy, setSortBy] = useState<'recent' | 'registered' | 'bookings' | 'listings'>('recent');
  ...
  const handleSuspendAccount = () => {
    console.log('Suspending account:', selectedUser?.id, 'for', suspendDuration, 'days');
  ...
  const getTypeBadge = (type: string) => {
    if (type === 'guest') {
      return <span className="px-3 py-1 bg-warm-green/20 text-warm-green text-xs font-bold rounded-full">
          Guest
        </span>;
    } else if (type === 'host') {
      return <span className="px-3 py-1 bg-terracotta/20 text-terracotta text-xs font-bold rounded-full">
          Host
        </span>;
    } else if (type === 'both') {
      return <span className="px-3 py-1 bg-gradient-to-r from-warm-green/20 to-terracotta/20 text-charcoal text-xs font-bold rounded-full">
          Guest + Host
        </span>;
    } else {
      return <span className="px-3 py-1 bg-charcoal/20 text-charcoal text-xs font-bold rounded-full">
          Admin
        </span>;
    }
  };
  ...
  return <div className="min-h-screen bg-cream pt-24 pb-12 px-6 md:px-12">
      <div className="max-w-[1800px] mx-auto">
        ...
        y: 0
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            ...
            <div className="flex items-center flex-wrap gap-3">
              ...
              <button onClick={() => setShowBulkEmailModal(true)} className="flex items-center space-x-2 px-6 py-3 bg-terracotta text-white rounded-lg hover:bg-terracotta/90 transition-all shadow-sm hover:-translate-y-0.5">
        ...
        delay: 0.1
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            <div className="lg:col-span-2 relative">
              ...
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by name, email, phone, or ID number..." className="w-full pl-12 pr-4 py-3 border border-charcoal/20 rounded-lg outline-none focus:border-warm-green transition-colors bg-white" />
            </div>

            {/* User Type - UPDATED */}
            <select value={userTypeFilter} onChange={e => setUserTypeFilter(e.target.value)} className="px-4 py-3 border border-charcoal/20 rounded-lg outline-none focus:border-warm-green transition-colors bg-white">
            ...
            {/* Sort By */}
            <select value={sortBy} onChange={e => setSortBy(e.target.value as any)} className="px-4 py-3 border border-charcoal/20 rounded-lg outline-none focus:border-warm-green transition-colors bg-white">
          ...
          <div className="flex items-center justify-between">
            <button onClick={() => setShowFilters(!showFilters)} className="flex items-center space-x-2 px-4 py-2 bg-white border border-charcoal/20 rounded-lg hover:border-warm-green transition-colors text-sm font-medium">
              <Filter className="w-4 h-4" />
              <span>Advanced Filters</span>
              {(verificationFilter.length > 0 || statusFilter.length > 0) && <span className="px-2 py-1 bg-warm-green text-white text-xs font-bold rounded-full">
                  Active
                </span>}
            </button>
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 pt-4 border-t border-charcoal/10">
                  <div>
                    ...
                    <div className="space-y-2">
                      {['verified', 'pending', 'rejected'].map(status => <label key={status} className="flex items-center space-x-2 cursor-pointer">
                          <input type="checkbox" checked={verificationFilter.includes(status)} onChange={e => {
                      if (e.target.checked) {
                        setVerificationFilter([...verificationFilter, status]);
                      } else {
                        setVerificationFilter(verificationFilter.filter(s => s !== status));
                      }
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {['active', 'suspended'].map(status => <label key={status} className="flex items-center space-x-2 cursor-pointer">
                  ...
                  <div>
                    ...
                    <input type="date" className="w-full px-4 py-2 border border-charcoal/20 rounded-lg outline-none focus:border-warm-green transition-colors bg-white text-sm" />
                  </div>
                </div>
              </motion.div>}
          </AnimatePresence>
        ...
        {/* Bulk Actions Bar */}
        <AnimatePresence>
          {selectedUsers.length > 0 && <motion.div initial={{
          opacity: 0,
          y: -20
        }} animate={{
          opacity: 1,
          ...
          y: -20
              ...
              <div className="flex items-center space-x-4">
                ...
                <button onClick={() => setSelectedUsers([])} className="text-sm text-charcoal/60 hover:text-charcoal">
                  Clear selection
                </button>
              </div>
              <div className="flex items-center flex-wrap gap-3">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-sand border-b border-charcoal/10">
                <tr>
                  <th className="px-6 py-4 text-left">
                    <input type="checkbox" checked={selectedUsers.length === MOCK_USERS.length} onChange={selectAllUsers} className="w-4 h-4 accent-warm-green" />
                  </th>
              ...
              <tbody className="divide-y divide-charcoal/5">
                {MOCK_USERS.map((user, index) => <motion.tr key={user.id} initial={{
                opacity: 0,
                y: 20
              }} animate={{
                opacity: 1,
                ...
                delay: 0.3 + index * 0.05
                    ...
                    <td className="px-6 py-4">
                      <input type="checkbox" checked={selectedUsers.includes(user.id)} onChange={() => toggleUserSelection(user.id)} className="w-4 h-4 accent-warm-green" />
                    </td>
                    <td className="px-6 py-4">
                      {user.avatar ? <img src={user.avatar} alt={user.name} className="w-10 h-10 rounded-full object-cover border-2 border-sand" /> : <div className="w-10 h-10 rounded-full bg-sand flex items-center justify-center border-2 border-sand">
                    ...
                    <td className="px-6 py-4">
                      <Link to={`/admin/users/${user.id}`} className="font-medium text-charcoal hover:text-warm-green hover:underline transition-colors">
                        {user.name}
                      </Link>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-start space-x-2">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${getVerificationBg(user.verification.status)}`}>
                          {user.verification.status === 'verified' && <CheckCircle className={`w-5 h-5 ${getVerificationColor(user.verification.status)}`} />}
                          {user.verification.status === 'pending' && <Clock className={`w-5 h-5 ${getVerificationColor(user.verification.status)}`} />}
                          {user.verification.status === 'rejected' && <XCircle className={`w-5 h-5 ${getVerificationColor(user.verification.status)}`} />}
                        </div>
                        <div>
                          ...
                          {user.verification.date && <p className="text-xs text-charcoal/50">
                              {user.verification.date}
                            </p>}
                          {user.verification.status === 'pending' && <button onClick={() => {
                    ...
                    <td className="px-6 py-4">
                      {user.type === 'guest' || user.type === 'both' ? <p className="text-sm text-charcoal">
                          <span className="font-medium text-warm-green">
                            {user.bookings}
                          </span>{' '}
                          bookings
                        </p> : null}
                      {user.type === 'host' || user.type === 'both' ? <div className="text-sm">
                          <p className="text-terracotta font-medium">
                            {user.properties} properties
                          </p>
                          {user.totalBookings && <p className="text-charcoal/60">
                    ...
                    <td className="px-6 py-4">
                      ...
                      {user.status === 'suspended' && user.suspendedUntil && <p className="text-xs text-charcoal/60 mt-1">
                    ...
                    <td className="px-6 py-4 relative">
                      <button onClick={() => setOpenMenuId(openMenuId === user.id ? null : user.id)} className="p-2 hover:bg-sand rounded-lg transition-colors">
                      ...
                      <AnimatePresence>
                        ...
                        duration: 0.15
                              ...
                              {user.status === 'suspended' ? <button onClick={() => {
                          setSelectedUser(user);
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={exportFilters.userType} onChange={e => setExportFilters({
                  ...exportFilters,
                  ...
                  <div className="col-span-2">
                    ...
                    <select value={exportFilters.verification} onChange={e => setExportFilters({
                  ...exportFilters,
      ...
      {/* Bulk Email Modal */}
      <AnimatePresence>
        {showBulkEmailModal && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          opacity: 0
        }} className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowBulkEmailModal(false)} />
            <motion.div initial={{
          opacity: 0,
          scale: 0.95,
          y: 20
          ...
          y: 20
              ...
              <div className="flex items-center justify-between p-6 border-b border-charcoal/10 sticky top-0 bg-white z-10">
                ...
                <button onClick={() => setShowBulkEmailModal(false)} className="p-2 hover:bg-charcoal/5 rounded-lg transition-colors">
                  <X className="w-5 h-5 text-charcoal" />
                </button>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={bulkEmailFilters.userType} onChange={e => setBulkEmailFilters({
                    ...bulkEmailFilters,
                    userType: e.target.value
                    ...
                    <div>
                      ...
                      <select value={bulkEmailFilters.status} onChange={e => setBulkEmailFilters({
                    ...bulkEmailFilters,
                    status: e.target.value
                  })} className="w-full px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta/50">
                        <option value="all">All Status</option>
                ...
                <div className="border-t border-charcoal/10 pt-6 space-y-4">
                  ...
                  <div>
                    ...
                    <input type="text" value={emailSubject} onChange={e => setEmailSubject(e.target.value)} placeholder="Email subject..." className="w-full px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta/50" />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea value={emailContent} onChange={e => setEmailContent(e.target.value)} placeholder="Write your email message here..." rows={8} className="w-full px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta/50 resize-none" />
                  </div>
                </div>
              </div>

              <div className="flex items-center justify-end space-x-3 p-6 border-t border-charcoal/10 bg-sand/30">
                <button onClick={resetBulkEmailFilters} className="flex items-center space-x-2 px-4 py-3 border border-charcoal/20 text-charcoal rounded-lg hover:bg-charcoal/5 transition-colors font-medium">
                  <RefreshCw className="w-4 h-4" />
                  <span>Reset</span>
                </button>
                <button onClick={() => setShowBulkEmailModal(false)} className="px-6 py-3 border border-charcoal/20 text-charcoal rounded-lg hover:bg-charcoal/5 transition-colors font-medium">
                  Cancel
                </button>
                <button onClick={handleBulkEmail} disabled={!emailSubject.trim() || !emailContent.trim()} className="px-6 py-3 bg-terracotta text-white rounded-lg hover:bg-terracotta/90 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed">
                  Send Email
                </button>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {showReviewDocsModal && selectedUser && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          y: 20
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-sand rounded-lg p-8 flex items-center justify-center min-h-[400px]">
                    {selectedUser.verification.documentUrl?.endsWith('.pdf') ? <div className="text-center">
                        <FileText className="w-20 h-20 text-charcoal/40 mx-auto mb-4" />
                        <p className="text-charcoal/60 mb-4">PDF Document</p>
                        <a href={selectedUser.verification.documentUrl} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-warm-green text-white rounded-lg hover:bg-warm-green/90 transition-colors text-sm font-medium inline-block">
                          Open PDF in New Tab
                        </a>
              ...
              setSelectedUser(null);
                ...
                <button onClick={handleRejectDocument} className="px-6 py-3 bg-terracotta text-white rounded-lg hover:bg-terracotta/90 transition-colors font-medium">
                  Reject Document
                </button>
                <button onClick={handleVerifyDocument} className="px-6 py-3 bg-warm-green text-white rounded-lg hover:bg-warm-green/90 transition-colors font-medium">
                  Verify Identity
                </button>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {showSuspendModal && selectedUser && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          y: 20
              ...
              <div className="mb-6">
                ...
                <select value={suspendDuration} onChange={e => setSuspendDuration(e.target.value)} className="w-full px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold/50">
              ...
              setSelectedUser(null);
                ...
                <button onClick={handleSuspendAccount} className="flex-1 px-6 py-3 bg-gold text-white rounded-lg hover:bg-gold/90 transition-colors font-medium">
                  Suspend Account
                </button>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {showResetPasswordModal && selectedUser && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          y: 20
              ...
              <div className="space-y-4 mb-6">
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <input type="text" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="Enter new password..." className="flex-1 px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-warm-green/50" />
                    <button onClick={generateStrongPassword} className="px-4 py-3 bg-warm-green text-white rounded-lg hover:bg-warm-green/90 transition-colors text-sm font-medium whitespace-nowrap">
                      Generate
                    </button>
                  </div>
                </div>
                ...
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input type="checkbox" checked={emailPassword} onChange={e => setEmailPassword(e.target.checked)} className="w-4 h-4 accent-warm-green" />
              ...
              setEmailPassword(false);
                ...
                <button onClick={handleResetPassword} disabled={!newPassword.trim()} className="flex-1 px-6 py-3 bg-warm-green text-white rounded-lg hover:bg-warm-green/90 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed">
                  Reset Password
                </button>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {showActivateModal && selectedUser && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          y: 20
              ...
              setSelectedUser(null);
                ...
                <button onClick={handleActivateAccount} className="flex-1 px-6 py-3 bg-warm-green text-white rounded-lg hover:bg-warm-green/90 transition-colors font-medium">
                  Activate Account
                </button>
              </div>
            </motion.div>
