...
import { Search, Download, FileText, Settings, Filter, AlertTriangle, Home, Users, DollarSign, Shield, Clock, CheckCircle, MapPin, Calendar, Eye, UserPlus, Mail, Ban, Flag, MoreVertical, TrendingUp, XCircle, X, RefreshCw, ChevronLeft, ChevronRight } from 'lucide-react';
// Mock report data - UPDATED (removed priority, simplified)
const MOCK_REPORTS = [{
  id: 'RPT-2024-0847',
  type: 'property',
  filedBy: {
...
export function AdminReportsPage() {
  ...
  const getTypeBadge = (type: string) => {
    ...
    return <span className={`px-3 py-1 ${config.bg} ${config.text} text-xs font-bold rounded-full flex items-center space-x-1 whitespace-nowrap`}>
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'awaiting-info':
        return <span className="px-3 py-1 bg-gold/70 text-white text-xs font-bold rounded-full">
            Awaiting Response
          </span>;
      case 'resolved':
  ...
  const getUserTypeBadge = (userType: string) => {
    if (userType === 'guest') {
      return <span className="px-2 py-0.5 bg-warm-green/20 text-warm-green text-xs font-bold rounded-full">
          Guest
        </span>;
    }
    return <span className="px-2 py-0.5 bg-terracotta/20 text-terracotta text-xs font-bold rounded-full">
        Host
      </span>;
  };
  const getBookingStatusBadge = (status: string) => {
    switch (status) {
      case 'completed':
        return <span className="px-2 py-0.5 bg-charcoal/60 text-white text-xs font-bold rounded-full">
            Completed
          </span>;
      case 'in-progress':
        return <span className="px-2 py-0.5 bg-warm-green text-white text-xs font-bold rounded-full">
            In Progress
          </span>;
      case 'upcoming':
        return <span className="px-2 py-0.5 bg-gold text-white text-xs font-bold rounded-full">
            Upcoming
          </span>;
      default:
        return null;
  ...
  return <div className="min-h-screen bg-cream pt-24 pb-12 px-6 md:px-12">
      <div className="max-w-[1800px] mx-auto">
        ...
        y: 0
          ...
          <div className="flex items-center space-x-3">
            ...
            <button onClick={() => setShowGuidelinesModal(true)} className="flex items-center space-x-2 px-6 py-3 bg-gold text-white rounded-lg hover:bg-gold/90 transition-all shadow-sm hover:-translate-y-0.5">
        ...
        delay: 0.5
          ...
          <div className="flex items-center space-x-4 mb-4">
            ...
            <div className="flex-1 relative">
              ...
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search reports by ID, user, or property..." className="w-full pl-12 pr-4 py-3 border border-charcoal/20 rounded-lg outline-none focus:border-terracotta transition-colors bg-white" />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="w-full px-4 py-2 border border-charcoal/20 rounded-lg outline-none focus:border-terracotta transition-colors bg-white text-sm">
            ...
            <div>
              ...
              <input type="date" className="w-full px-4 py-2 border border-charcoal/20 rounded-lg outline-none focus:border-terracotta transition-colors bg-white text-sm" />
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-charcoal/5">
                {MOCK_REPORTS.map((report, index) => <motion.tr key={report.id} initial={{
                opacity: 0,
                y: 20
              }} animate={{
                opacity: 1,
                ...
                delay: 0.7 + index * 0.05
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-3">
                        {report.filedBy.avatar ? <img src={report.filedBy.avatar} alt={report.filedBy.name} className="w-10 h-10 rounded-full border-2 border-sand" /> : <div className="w-10 h-10 rounded-full bg-warm-green/10 flex items-center justify-center border-2 border-sand">
                            <span className="text-sm font-bold text-warm-green">
                              {report.filedBy.initials}
                            </span>
                          </div>}
                        <div>
                          <Link to={`/admin/users/${report.id}`} className="text-sm font-medium text-charcoal hover:text-warm-green transition-colors">
                            {report.filedBy.name}
                          </Link>
                          <div className="mt-1">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-3">
                        {report.against.avatar ? <img src={report.against.avatar} alt={report.against.name} className="w-10 h-10 rounded-full border-2 border-sand" /> : <div className="w-10 h-10 rounded-full bg-terracotta/10 flex items-center justify-center border-2 border-sand">
                    ...
                    <td className="px-6 py-4">
                      <div>
                        <Link to={`/admin/reservations/${report.booking.id}`} className="text-sm font-mono text-warm-green hover:underline">
                          {report.booking.id}
                        </Link>
                        ...
                        <Link to={`/property/${report.property.id}`} className="text-xs text-terracotta hover:underline mt-2 flex items-center">
                    ...
                    <td className="px-6 py-4 relative">
                      <button onClick={() => setOpenMenuId(openMenuId === report.id ? null : report.id)} className="p-2 hover:bg-sand rounded-lg transition-colors">
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select value={exportFilters.filedBy} onChange={e => setExportFilters({
                  ...exportFilters,
      ...
      {/* Guidelines Modal */}
      <AnimatePresence>
        {showGuidelinesModal && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          opacity: 0
        }} className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowGuidelinesModal(false)} />
            <motion.div initial={{
          opacity: 0,
          scale: 0.95,
          y: 20
          ...
          y: 20
              ...
              <div className="flex items-center justify-between p-6 border-b border-charcoal/10 sticky top-0 bg-white z-10">
                ...
                <button onClick={() => setShowGuidelinesModal(false)} className="p-2 hover:bg-charcoal/5 rounded-lg transition-colors">
                  <X className="w-5 h-5 text-charcoal" />
                </button>
              </div>
              ...
              <div className="flex items-center justify-end p-6 border-t border-charcoal/10 bg-sand/30">
                <button onClick={() => setShowGuidelinesModal(false)} className="px-6 py-3 bg-warm-green text-white rounded-lg hover:bg-warm-green/90 transition-colors font-medium">
                  Got It
                </button>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {showFullReportModal && selectedReport && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
                ...
                <div className="border-t border-charcoal/10 pt-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-sand rounded-lg p-4">
                      ...
                      <div className="flex items-center space-x-3">
                        {selectedReport.filedBy.avatar ? <img src={selectedReport.filedBy.avatar} alt={selectedReport.filedBy.name} className="w-10 h-10 rounded-full" /> : <div className="w-10 h-10 rounded-full bg-warm-green/10 flex items-center justify-center">
                            <span className="text-sm font-bold text-warm-green">
                    ...
                    <div className="bg-sand rounded-lg p-4">
                      ...
                      <div className="flex items-center space-x-3">
                        {selectedReport.against.avatar ? <img src={selectedReport.against.avatar} alt={selectedReport.against.name} className="w-10 h-10 rounded-full" /> : <div className="w-10 h-10 rounded-full bg-terracotta/10 flex items-center justify-center">
                            <span className="text-sm font-bold text-terracotta">
                ...
                <div className="border-t border-charcoal/10 pt-4">
                  ...
                  <div className="bg-sand rounded-lg p-4 space-y-3">
                    ...
                    <div>
                      ...
                      <Link to={`/property/${selectedReport.property.id}`} className="text-sm text-terracotta hover:underline font-medium">
      ...
      <AnimatePresence>
        {showRequestInfoModal && selectedReport && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          y: 20
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea value={requestInfoMessage} onChange={e => setRequestInfoMessage(e.target.value)} placeholder="Write your request for additional information here..." rows={6} className="w-full px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold/50 resize-none" />
                </div>
              ...
              setRequestInfoMessage('');
                ...
                <button onClick={handleRequestInfo} disabled={!requestInfoMessage.trim()} className="px-6 py-3 bg-gold text-white rounded-lg hover:bg-gold/90 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed">
                  Send Request
                </button>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {showCancelReservationModal && selectedReport && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
