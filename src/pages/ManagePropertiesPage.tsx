...
import { Plus, Search, Filter, Edit, Trash2, Eye, MoreVertical, DollarSign, Calendar, TrendingUp, ChevronDown, X } from 'lucide-react';
const MOCK_PROPERTIES = [{
  ...
  image: 'https://images.unsplash.com/photo-1600596542815-2495db9dc2c3?auto=format&fit=crop&q=80&w=600',
  status: 'active',
  ...
  image: 'https://images.unsplash.com/photo-1513581166391-887a96ddeafd?auto=format&fit=crop&q=80&w=600',
  status: 'inactive',
  monthlyEarnings: 0,
  totalBookings: 0,
  occupancyRate: 0,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        y: 0
          ...
          <div>
            ...
            <p className="text-xl text-charcoal/70 font-light">
              Manage your portfolio of {MOCK_PROPERTIES.length} beautiful spaces
            </p>
          </div>
          <Link to="/landlord/properties/new" className="flex items-center space-x-2 px-6 py-3 bg-warm-green text-white rounded-lg hover:bg-warm-green/90 transition-all shadow-lg shadow-warm-green/20 hover:-translate-y-1">
        ...
        delay: 0.1
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as 'all' | 'active' | 'inactive')} className="px-4 py-3 border border-charcoal/20 rounded-lg outline-none focus:border-warm-green transition-colors bg-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <AnimatePresence mode="popLayout">
            {filteredProperties.map((property, index) => <motion.div key={property.id} layout initial={{
            opacity: 0,
            scale: 0.9
          }} animate={{
            opacity: 1,
            ...
            delay: index * 0.05
                ...
                <div className="flex">
                  ...
                  <div className="w-1/3 relative">
                    ...
                    <div className={`absolute top-3 left-3 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${property.status === 'active' ? 'bg-warm-green text-white' : 'bg-charcoal/60 text-white'}`}>
                      {property.status}
                    </div>
                  </div>
                  ...
                  <div className="flex-1 p-6">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="relative">
                        <button onClick={() => setOpenMenuId(openMenuId === property.id ? null : property.id)} className="p-2 hover:bg-sand rounded-lg transition-colors">
                          <MoreVertical className="w-5 h-5 text-charcoal/60" />
                        </button>

                        <AnimatePresence>
                          {openMenuId === property.id && <motion.div initial={{
                        opacity: 0,
                        y: 10
                      }} animate={{
                        opacity: 1,
                        ...
                        y: 10
                              ...
                              <Link to={`/landlord/properties/${property.id}`} className="flex items-center space-x-3 px-4 py-2 hover:bg-sand transition-colors">
                                <Eye className="w-4 h-4 text-charcoal/60" />
                                <span className="text-sm">View Details</span>
                              </Link>
                              <Link to={`/landlord/properties/${property.id}/edit`} className="flex items-center space-x-3 px-4 py-2 hover:bg-sand transition-colors">
                                <Edit className="w-4 h-4 text-charcoal/60" />
                                <span className="text-sm">Edit Property</span>
                              </Link>
                              <button onClick={() => handleDelete(property.id)} className="w-full flex items-center space-x-3 px-4 py-2 hover:bg-red-50 transition-colors text-red-600">
        ...
        <AnimatePresence>
          {showDeleteModal && <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
              <motion.div initial={{
            opacity: 0
          }} animate={{
            opacity: 1
            ...
            y: 20
                ...
                <button onClick={() => setShowDeleteModal(false)} className="absolute top-4 right-4 p-2 hover:bg-sand rounded-lg transition-colors">
                ...
                <div className="text-center">
                  ...
                  <div className="flex space-x-4">
                    <button onClick={() => setShowDeleteModal(false)} className="flex-1 px-6 py-3 border border-charcoal/20 rounded-lg hover:border-charcoal/40 transition-colors font-medium">
                      Cancel
                    </button>
                    <button onClick={confirmDelete} className="flex-1 px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-medium">
                      Delete
                    </button>
                  </div>
                </div>
