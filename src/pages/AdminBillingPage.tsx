...
import { DollarSign, Percent, Clock, CreditCard, Download, FileText, Settings, TrendingUp, Users, Home, Calendar, CheckCircle, AlertCircle, Eye, Play, Ban, Edit, Filter, X, ChevronLeft, ChevronRight, RefreshCw, Plus, Search } from 'lucide-react';
// Mock data - Property-level aggregated commission data
...
// Host-focused refunds (not guest refunds)
const REFUNDS_DATA = [{
  id: 'REF-H-001',
  host: {
    name: 'Marco Rossi',
...
export function AdminBillingPage() {
  ...
  const [activeTab, setActiveTab] = useState<'commission' | 'analytics' | 'refunds'>('commission');
      ...
      <div className="max-w-[1800px] mx-auto">
        ...
        y: 0
          ...
          <div className="flex items-center space-x-3">
            <button onClick={handleExportReport} className="flex items-center space-x-2 px-6 py-3 bg-warm-green text-white rounded-lg hover:bg-warm-green/90 transition-all shadow-sm hover:-translate-y-0.5">
              <Download className="w-5 h-5" />
              <span className="font-medium">Export Financial Report</span>
            </button>
            <button onClick={handlePaymentSettings} className="flex items-center space-x-2 px-6 py-3 bg-gold text-white rounded-lg hover:bg-gold/90 transition-all shadow-sm hover:-translate-y-0.5">
        ...
        delay: 0.5
          ...
          <button onClick={() => setShowFilters(!showFilters)} className="flex items-center space-x-2 px-4 py-2 bg-white border border-charcoal/10 rounded-lg hover:bg-sand/50 transition-colors">
            <Filter className="w-5 h-5 text-charcoal" />
            <span className="font-medium text-charcoal">Filters</span>
            {Object.values(filters).some(v => v && v !== 'all') && <span className="px-2 py-1 bg-gold text-white text-xs font-bold rounded-full">
                Active
              </span>}
          </button>
            ...
            height: 0
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <input type="text" value={filters.hostName} onChange={e => setFilters({
                  ...filters,
                  hostName: e.target.value
                })} placeholder="Search by host name..." className="w-full px-4 py-2 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold/50" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-charcoal mb-2">
                      Property Name
                    </label>
                    <input type="text" value={filters.propertyName} onChange={e => setFilters({
                  ...filters,
                  propertyName: e.target.value
                })} placeholder="Search by property name..." className="w-full px-4 py-2 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold/50" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-charcoal mb-2">
                      Min Gross Revenue
                    </label>
                    <input type="number" value={filters.grossAmountMin} onChange={e => setFilters({
                  ...filters,
                  grossAmountMin: e.target.value
                })} placeholder="0" className="w-full px-4 py-2 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold/50" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-charcoal mb-2">
                      Max Gross Revenue
                    </label>
                    <input type="number" value={filters.grossAmountMax} onChange={e => setFilters({
                  ...filters,
                  grossAmountMax: e.target.value
                })} placeholder="999999" className="w-full px-4 py-2 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold/50" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-charcoal mb-2">
                      Status
        ...
        delay: 0.6
          ...
          <div className="flex items-center space-x-8 border-b border-charcoal/10">
            ...
            label: 'Host Refunds'
                ...
                {tab.label}
                {activeTab === tab.id && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-gold" />}
              </button>)}
          </div>
        </motion.div>

        {/* Commission Ledger Tab - PROPERTY-LEVEL AGGREGATION */}
        {activeTab === 'commission' && <motion.div initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        y: 0
            ...
            <div className="bg-warm-green/10 border border-warm-green/20 rounded-xl p-6">
              ...
              <p className="text-xs text-charcoal/60 mt-4">
                ...
                <Link to="/admin/reservations" className="text-warm-green underline font-medium">
                  Reservations page
                </Link>
                .
              </p>
            ...
            <div className="bg-white rounded-2xl border border-charcoal/5 shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-charcoal/5">
                    {paginatedData.map((item, index) => <tr key={item.id} className={`hover:bg-sand/50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-cream'}`}>
                        <td className="px-6 py-4">
                          <div className="flex items-center space-x-3">
                            {item.host.avatar ? <img src={item.host.avatar} alt={item.host.name} className="w-10 h-10 rounded-full" /> : <div className="w-10 h-10 rounded-full bg-gold/10 flex items-center justify-center">
                                <span className="text-sm font-bold text-gold">
                                  {item.host.initials}
                                </span>
                              </div>}
                            <div>
                              <Link to={`/admin/users/${item.host.name}`} className="font-medium text-charcoal hover:text-gold transition-colors">
                                {item.host.name}
                              </Link>
                              {item.host.verified && <p className="text-xs text-gold flex items-center">
                        ...
                        <td className="px-6 py-4">
                          <div>
                            <Link to={`/admin/properties/${item.property.id}`} className="font-medium text-charcoal hover:text-warm-green transition-colors">
                              {item.property.name}
                            </Link>
              ...
              <div className="flex items-center justify-between px-6 py-4 border-t border-charcoal/10 bg-sand/30">
                ...
                <div className="flex items-center space-x-2">
                  <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 border border-charcoal/20 rounded-lg hover:bg-sand transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    <ChevronLeft className="w-5 h-5 text-charcoal" />
                  </button>
                  <span className="px-4 py-2 text-sm font-medium text-charcoal">
                    Page {currentPage} of {totalPages}
                  </span>
                  <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2 border border-charcoal/20 rounded-lg hover:bg-sand transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        {activeTab === 'refunds' && <motion.div initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        y: 0
            ...
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowRefundForm(true)} className="flex items-center space-x-2 px-4 py-2 bg-terracotta text-white rounded-lg hover:bg-terracotta/90 transition-all">
            ...
            <AnimatePresence>
              {showRefundForm && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
                  <motion.div initial={{
              opacity: 0
            }} animate={{
              opacity: 1
              ...
              opacity: 0
            }} className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowRefundForm(false)} />
                  <motion.div initial={{
              opacity: 0,
              scale: 0.95,
              y: 20
              ...
              y: 20
                    ...
                    <div className="flex items-center justify-between p-6 border-b border-charcoal/10">
                      ...
                      <button onClick={() => setShowRefundForm(false)} className="p-2 hover:bg-charcoal/5 rounded-lg transition-colors">
                    ...
                    <div className="p-6 space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <input type="text" value={refundForm.hostName} onChange={e => setRefundForm({
                      ...refundForm,
                      hostName: e.target.value
                    })} placeholder="Search or select host..." className="w-full px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta/50" />
                        </div>
                        ...
                        <div>
                          ...
                          <input type="text" value={refundForm.propertyName} onChange={e => setRefundForm({
                      ...refundForm,
                      propertyName: e.target.value
                    })} placeholder="Select property..." className="w-full px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta/50" />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <input type="number" value={refundForm.amount} onChange={e => setRefundForm({
                      ...refundForm,
                      amount: e.target.value
                    })} placeholder="0.00" className="w-full px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta/50" />
                        </div>
                        ...
                        <div>
                          ...
                          <input type="number" value={refundForm.affectedBookings} onChange={e => setRefundForm({
                      ...refundForm,
                      affectedBookings: e.target.value
                    })} placeholder="1" className="w-full px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta/50" />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <textarea value={refundForm.reason} onChange={e => setRefundForm({
                    ...refundForm,
                    reason: e.target.value
                  })} placeholder="Explain the reason for this host refund (e.g., guest cancellation, property issue, billing error)..." rows={4} className="w-full px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta/50 resize-none" />
                      </div>
                    ...
                    <div className="flex items-center justify-end space-x-3 p-6 border-t border-charcoal/10 bg-sand/30">
                      <button onClick={() => setShowRefundForm(false)} className="px-6 py-3 border border-charcoal/20 text-charcoal rounded-lg hover:bg-charcoal/5 transition-colors font-medium">
                        Cancel
                      </button>
                      <button onClick={handleRefundSubmit} disabled={!refundForm.hostName || !refundForm.propertyName || !refundForm.amount || !refundForm.reason} className="px-6 py-3 bg-terracotta text-white rounded-lg hover:bg-terracotta/90 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed">
                        Process Host Refund
                      </button>
                    </div>
                  </motion.div>
            ...
            <div className="bg-white rounded-2xl border border-charcoal/5 shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-charcoal/5">
                    {REFUNDS_DATA.map((refund, index) => <tr key={refund.id} className={`hover:bg-sand/50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-cream'}`}>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center space-x-3">
                            {refund.host.avatar ? <img src={refund.host.avatar} alt={refund.host.name} className="w-10 h-10 rounded-full" /> : <div className="w-10 h-10 rounded-full bg-gold/10 flex items-center justify-center">
                                <span className="text-sm font-bold text-gold">
                            ...
                            <div>
                              ...
                              {refund.host.verified && <p className="text-xs text-gold flex items-center">
                                  <CheckCircle className="w-3 h-3 mr-1" />
                                  Verified
                                </p>}
                            </div>
        ...
        {activeTab === 'analytics' && <motion.div initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        y: 0
            ...
            <div className="bg-white rounded-2xl p-8 border border-charcoal/5 shadow-sm">
              ...
              const maxValue = Math.max(...MONTHLY_REVENUE.map(m => m.revenue));
