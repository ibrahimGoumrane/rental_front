...
import { Search, Download, FileText, Calendar, Filter, MoreVertical, CheckCircle, Clock, XCircle, AlertCircle, DollarSign, MapPin, Users, Shield, MessageSquare, Ban, RefreshCw, Eye, Flag, CreditCard, X, ChevronLeft, ChevronRight } from 'lucide-react';
// Mock reservation data - UPDATED
...
export function AdminReservationsPage() {
  ...
  const [selectedReservation, setSelectedReservation] = useState<(typeof MOCK_RESERVATIONS)[0] | null>(null);
  const stats = {
    active: 1892,
    disputes: 15,
    totalValue: 284000
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'cancelled-guest':
        return <span className="px-3 py-1 bg-gold/80 text-white text-xs font-bold rounded-full">
            Cancelled by Guest
          </span>;
      case 'cancelled-host':
        return <span className="px-3 py-1 bg-terracotta/80 text-white text-xs font-bold rounded-full">
            Cancelled by Host
          </span>;
      case 'checked-in':
        return <span className="px-3 py-1 bg-warm-green text-white text-xs font-bold rounded-full animate-pulse">
            Checked In
          </span>;
      case 'completed':
        return <span className="px-3 py-1 bg-charcoal/60 text-white text-xs font-bold rounded-full">
            Completed
          </span>;
      case 'disputed':
  ...
  return <div className="min-h-screen bg-cream pt-24 pb-12 px-6 md:px-12">
        ...
        delay: 0.1
          ...
          <div className="flex items-center space-x-4 mb-4">
            ...
            <div className="flex-1 relative">
              ...
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by booking ID or property name..." className="w-full pl-12 pr-4 py-3 border border-charcoal/20 rounded-lg outline-none focus:border-warm-green transition-colors bg-white" />
            </div>

            <button onClick={() => setShowFilters(!showFilters)} className="flex items-center space-x-2 px-4 py-3 bg-white border border-charcoal/20 rounded-lg hover:border-warm-green transition-colors">
              <Filter className="w-5 h-5" />
              <span className="font-medium">Filters</span>
              {(statusFilter !== 'all' || startDate || endDate || guestName) && <span className="px-2 py-1 bg-warm-green text-white text-xs font-bold rounded-full">
                  Active
                </span>}
            </button>
          </div>
            ...
            duration: 0.3
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t border-charcoal/10">
                  <div>
                    ...
                    <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="w-full px-4 py-2 border border-charcoal/20 rounded-lg outline-none focus:border-warm-green transition-colors bg-white text-sm">
                  ...
                  <div>
                    ...
                    <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full px-4 py-2 border border-charcoal/20 rounded-lg outline-none focus:border-warm-green transition-colors bg-white text-sm" />
                  </div>
                  ...
                  <div>
                    ...
                    <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full px-4 py-2 border border-charcoal/20 rounded-lg outline-none focus:border-warm-green transition-colors bg-white text-sm" />
                  </div>
                  ...
                  <div>
                    ...
                    <input type="text" value={guestName} onChange={e => setGuestName(e.target.value)} placeholder="Filter by guest..." className="w-full px-4 py-2 border border-charcoal/20 rounded-lg outline-none focus:border-warm-green transition-colors bg-white text-sm" />
                  </div>
                </div>

                <div className="flex items-center justify-end space-x-3 mt-4 pt-4 border-t border-charcoal/10">
                  <button onClick={resetFilters} className="flex items-center space-x-2 px-4 py-2 border border-charcoal/20 text-charcoal rounded-lg hover:bg-charcoal/5 transition-colors text-sm font-medium">
                    <RefreshCw className="w-4 h-4" />
                    <span>Reset</span>
                  </button>
                  <button onClick={() => setShowFilters(false)} className="px-4 py-2 bg-warm-green text-white rounded-lg hover:bg-warm-green/90 transition-colors text-sm font-medium">
                    Apply Filters
                  </button>
                </div>
              </motion.div>}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-charcoal/5">
                {MOCK_RESERVATIONS.map((reservation, index) => <motion.tr key={reservation.id} initial={{
                opacity: 0,
                y: 20
              }} animate={{
                opacity: 1,
                ...
                delay: 0.3 + index * 0.05
              }} className={`hover:bg-sand/50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-cream'} ${reservation.daysUntil >= 0 && reservation.daysUntil < 1 && reservation.status === 'confirmed' ? 'border-l-4 border-terracotta' : ''}`}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-3">
                        <img src={reservation.property.image} alt={reservation.property.name} className="w-16 h-16 rounded-lg object-cover" />
                        <div>
                          <Link to={`/property/${reservation.property.id}`} className="font-bold text-charcoal hover:text-terracotta transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-2">
                        {reservation.guest.avatar ? <img src={reservation.guest.avatar} alt={reservation.guest.name} className="w-8 h-8 rounded-full" /> : <div className="w-8 h-8 rounded-full bg-warm-green/10 flex items-center justify-center">
                        ...
                        <div>
                          <div className="flex items-center space-x-1">
                            <Link to={`/admin/users/${reservation.id}`} className="text-sm font-medium text-charcoal hover:text-warm-green transition-colors">
                              {reservation.guest.name}
                            </Link>
                            {reservation.guest.verified && <Shield className="w-3 h-3 text-gold" />}
                          </div>
                          <p className={`text-xs ${reservation.guest.previousBookings > 10 ? 'text-warm-green' : 'text-charcoal/60'}`}>
                    ...
                    <td className="px-6 py-4">
                      ...
                      {reservation.daysUntil >= 0 && <p className={`text-xs mt-1 ${reservation.daysUntil < 3 ? 'text-gold font-medium' : 'text-charcoal/60'}`}>
                          {reservation.daysUntil === 0 ? 'Today' : `In ${reservation.daysUntil} days`}
                        </p>}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 relative">
                      <button onClick={() => setOpenMenuId(openMenuId === reservation.id ? null : reservation.id)} className="p-2 hover:bg-sand rounded-lg transition-colors">
                      ...
                      <AnimatePresence>
                        ...
                        duration: 0.15
                              ...
                              <Link to={`/admin/messages`} className="w-full flex items-center space-x-3 px-4 py-2 hover:bg-sand transition-colors text-sm">
                          ...
                          setOpenMenuId(null);
                              ...
                              {reservation.status === 'disputed' && <button className="w-full flex items-center space-x-3 px-4 py-2 hover:bg-sand transition-colors text-sm text-warm-green">
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="text" value={exportFilters.guestName} onChange={e => setExportFilters({
                  ...exportFilters,
                  guestName: e.target.value
                })} placeholder="Filter by guest..." className="w-full px-4 py-3 border border-charcoal/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-warm-green/50" />
                  </div>
                  ...
                  <div>
                    ...
                    <input type="date" value={exportFilters.startDate} onChange={e => setExportFilters({
                  ...exportFilters,
                  ...
                  <div>
                    ...
                    <input type="date" value={exportFilters.endDate} onChange={e => setExportFilters({
                  ...exportFilters,
      ...
      <AnimatePresence>
        {showFlagModal && selectedReservation && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          y: 20
                ...
                <div className="space-y-4">
                  <div className="bg-sand rounded-lg p-4">
                    ...
                    <div className="flex items-center space-x-3 mb-3">
                      <img src={selectedReservation.property.image} alt={selectedReservation.property.name} className="w-20 h-20 rounded-lg object-cover" />
                      <div>
                        ...
                        <Link to={`/property/${selectedReservation.property.id}`} className="text-xs text-terracotta hover:underline mt-1 inline-block">
                          View Property Page
                        </Link>
                      </div>
                    </div>
              ...
              setSelectedReservation(null);
                ...
                <button onClick={handleFlagReservation} className="px-6 py-3 bg-gold text-white rounded-lg hover:bg-gold/90 transition-colors font-medium">
                  Flag for Review
                </button>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {showCancelModal && selectedReservation && <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          y: 20
              ...
              setSelectedReservation(null);
                ...
                <button onClick={handleCancelReservation} className="flex-1 px-6 py-3 bg-terracotta text-white rounded-lg hover:bg-terracotta/90 transition-colors font-medium">
