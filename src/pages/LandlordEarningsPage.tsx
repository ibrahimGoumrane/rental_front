...
import { DollarSign, TrendingUp, TrendingDown, Calendar, Download, Filter, Home, ChevronDown } from 'lucide-react';
...
const PROPERTY_EARNINGS = [{
  ...
  image: 'https://images.unsplash.com/photo-1600596542815-2495db9dc2c3?auto=format&fit=crop&q=80&w=200',
  bookings: 8,
  gross: 19600,
  commission: 2352,
  net: 17248,
...
export function LandlordEarningsPage() {
  ...
  const percentageChange = (MOCK_EARNINGS_DATA.thisMonth - MOCK_EARNINGS_DATA.lastMonth) / MOCK_EARNINGS_DATA.lastMonth * 100;
  const maxEarning = Math.max(...MONTHLY_EARNINGS.map(m => m.gross));
  // Filter property earnings
  const filteredEarnings = filterBy === 'property' && selectedProperty ? PROPERTY_EARNINGS.filter(p => p.id === selectedProperty) : PROPERTY_EARNINGS;
  const totalFiltered = filteredEarnings.reduce((acc, p) => ({
  ...
  return <div className="min-h-screen bg-cream pt-24 pb-12 px-6 md:px-12">
      <div className="max-w-7xl mx-auto">
        ...
        delay: 0.1
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center space-x-4">
              ...
              }} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${filterBy === 'all' ? 'bg-warm-green text-white' : 'bg-sand text-charcoal hover:bg-sand/80'}`}>
                  All Properties
                </button>
                <div className="relative">
                  <button onClick={() => setShowFilterDropdown(!showFilterDropdown)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center space-x-2 ${filterBy === 'property' ? 'bg-warm-green text-white' : 'bg-sand text-charcoal hover:bg-sand/80'}`}>
                    <span>By Property</span>
                    <ChevronDown className="w-4 h-4" />
                  </button>

                  {showFilterDropdown && <motion.div initial={{
                  opacity: 0,
                  y: 10
                }} animate={{
                  opacity: 1,
                  y: 0
                      ...
                      {PROPERTY_EARNINGS.map(property => <button key={property.id} onClick={() => {
                    setFilterBy('property');
                    setSelectedProperty(property.id);
                    setShowFilterDropdown(false);
                  }} className={`w-full text-left px-4 py-3 hover:bg-sand transition-colors flex items-center space-x-3 ${selectedProperty === property.id ? 'bg-sand/50' : ''}`}>
                          <img src={property.image} alt={property.name} className="w-10 h-10 rounded-lg object-cover" />
                          <div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <select value={timeRange} onChange={e => setTimeRange(e.target.value as '6months' | '1year' | 'all')} className="px-4 py-2 bg-sand border border-charcoal/10 rounded-lg text-sm font-medium text-charcoal outline-none focus:border-warm-green transition-colors">
        ...
        {/* Stats Cards */}
        <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12" initial="hidden" animate="show" variants={{
        hidden: {
          opacity: 0
        },
        show: {
          ...
          show: {
            ...
            <div className={`flex items-center text-sm ${percentageChange >= 0 ? 'text-warm-green' : 'text-terracotta'}`}>
        ...
        delay: 0.3
          ...
          <div className="space-y-6">
            {MONTHLY_EARNINGS.map((month, index) => <motion.div key={month.month} initial={{
            opacity: 0,
            x: -20
          }} animate={{
            opacity: 1,
        ...
        delay: 0.4
          ...
          <div className="flex items-center justify-between mb-8">
            <h2 className="font-serif text-3xl text-charcoal">
              {filterBy === 'property' && selectedProperty ? 'Property Performance' : 'Property Breakdown'}
            </h2>
            {filterBy === 'property' && <div className="text-right">
          ...
          <div className="space-y-4">
            {filteredEarnings.map((property, index) => <motion.div key={property.id} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
